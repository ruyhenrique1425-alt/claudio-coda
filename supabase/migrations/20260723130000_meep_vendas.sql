...
CREATE TABLE IF NOT EXISTS public.meep_vendas_bar (
  ...
  cartao     text,                        -- identificador do cartão do bar (MEEP)
  data       date NOT NULL,               -- dia da venda
  categoria  text,
  produto    text NOT NULL,
  quantidade numeric NOT NULL DEFAULT 0,  -- unidades vendidas (chopp = barris/copos)
  valor      numeric NOT NULL DEFAULT 0,
  is_chopp   boolean NOT NULL DEFAULT false,
  created_at timestamptz NOT NULL DEFAULT now()
);

-- Idempotência na importação: uma linha por (cartão, dia, produto).
