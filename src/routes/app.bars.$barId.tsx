...
import { ArrowLeft, Camera, Loader2, Plus, Trash2, Save, Thermometer, Snowflake, ClipboardCheck, Award, Users, CreditCard, CheckCircle2, AlertTriangle, ArrowRightLeft, CloudOff, RotateCw } from "lucide-react";
...
export const Route = createFileRoute("/app/bars/$barId")({
  validateSearch: (s: Record<string, unknown>) => ({
    tab: (TAB_KEYS as string[]).includes(s.tab as string) ? (s.tab as TabKey) : ("inventario" as TabKey),
  }),
  component: BarDetail,
  pendingComponent: () => (
    <div className="p-8 text-center text-muted-foreground">
      <Loader2 className="inline animate-spin mr-2" />Carregando bar…
    </div>
  ),
    ...
    return (
      <div className="mx-auto max-w-md p-6 text-center space-y-3">
        ...
        <div className="flex justify-center gap-2">
          ...
          <Button variant="outline" asChild><Link to="/app">Voltar</Link></Button>
        </div>
      </div>
    );
  },
...
function BarDetail() {
  ...
  async function loadAll() {
    ...
    setLoadError(null);
    try {
      const [{ data: b }, { data: m }, { data: c }, { data: s }, { data: ss }, { data: inv }, { data: rf }, { data: em }] =
        await Promise.all([
          ...
          supabase.from("inventories").select("*, inventory_items(*)").eq("bar_id", barId).order("performed_at", { ascending: false }).limit(10),
          supabase.from("refills").select("*, refill_items(*)").eq("bar_id", barId).order("performed_at", { ascending: false }).limit(10),
          supabase.from("empties_removed").select("*").eq("bar_id", barId).order("performed_at", { ascending: false }).limit(50),
        ]);
      setBar(b);
      setMachines(m ?? []);
  ...
  useEffect(() => { loadAll(); /* eslint-disable-next-line */ }, [barId]);

  if (loading) return <div className="p-8 text-center text-muted-foreground"><Loader2 className="inline animate-spin mr-2" />Carregando…</div>;
  if (loadError) return (
    <div className="mx-auto max-w-md p-6 text-center space-y-3">
      <AlertTriangle className="w-8 h-8 text-destructive mx-auto" />
      ...
      <div className="flex justify-center gap-2">
        <Button onClick={loadAll}><RotateCw className="w-4 h-4 mr-1" /> Tentar novamente</Button>
        <Button variant="outline" asChild><Link to="/app">Voltar</Link></Button>
      </div>
    </div>
  );
  if (!bar) return <div className="p-8 text-center">Bar não encontrado. <Link to="/app" className="underline">Voltar</Link></div>;

  // ---------- consumption computed from empties ----------
  const consumo: Record<Brand, number> = { heineken: 0, amstel: 0 };
  empties.forEach((e: any) => { consumo[e.brand as Brand] = (consumo[e.brand as Brand] ?? 0) + e.quantidade; });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-4 space-y-4">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => nav({ to: "/app" })}><ArrowLeft className="w-4 h-4 mr-1" />Voltar</Button>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {bar.apoio_responsavel && <span className="truncate">Apoio: <b>{bar.apoio_responsavel}</b></span>}
          </div>
        </div>
      </div>

      <Tabs value={tab} onValueChange={(v) => nav({ to: "/app/bars/$barId", params: { barId }, search: { tab: v as TabKey }, replace: true })}>
        <TabsList className="w-full grid grid-cols-6">
          <TabsTrigger value="inventario" className="font-display text-[11px] tracking-wider">INVENT.</TabsTrigger>
          <TabsTrigger value="reposicao" className="font-display text-[11px] tracking-wider">REPOR</TabsTrigger>
          <TabsTrigger value="missoes" className="font-display text-[11px] tracking-wider">MISSÕES</TabsTrigger>
          <TabsTrigger value="equipe" className="font-display text-[11px] tracking-wider">EQUIPE</TabsTrigger>
          <TabsTrigger value="consumo" className="font-display text-[11px] tracking-wider">CONSUMO</TabsTrigger>
          <TabsTrigger value="config" className="font-display text-[11px] tracking-wider">CONFIG</TabsTrigger>
        </TabsList>

        <TabsContent value="inventario" className="mt-3">
          <InventorySection barId={barId} stockStd={stockStd} inventories={inventories} onDone={loadAll} lastCounts={lastCounts} />
        </TabsContent>

        <TabsContent value="reposicao" className="mt-3 space-y-3">
          <RefillSection barId={barId} refills={refills} onDone={loadAll} stockStd={stockStd} lastCounts={lastCounts} />
        ...
        <TabsContent value="consumo" className="mt-3">
          <Card className="p-4">
            <h2 className="font-display text-sm tracking-widest text-muted-foreground mb-3">VAZIOS RETIRADOS</h2>
        ...
        <TabsContent value="config" className="mt-3">
          <Card className="p-4 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-display text-sm tracking-widest text-muted-foreground">CONFIGURAÇÃO</h2>
            ...
            <MachinesEditor barId={barId} machines={machines} isGestor={isGestor} onChanged={loadAll} />
            <InfraEditor bar={bar} isGestor={isGestor} onSaved={loadAll} />
            <div className="grid gap-4 md:grid-cols-3">
              <CardReaderEditor barId={barId} qty={cardQty} isGestor={isGestor} onChanged={loadAll} />
              <ShiftsEditor barId={barId} shifts={shifts} isGestor={isGestor} onChanged={loadAll} />
              <StockStdEditor barId={barId} std={stockStd} isGestor={isGestor} onChanged={loadAll} />
            </div>
          </Card>
        </TabsContent>
      </Tabs>
...
function BarBasics({ bar, isGestor, onSaved }: any) {
  ...
  async function save() {
    ...
    const { error } = await supabase.from("bars").update({
      name, apoio_responsavel: apoio || null, notes: notes || null,
    }).eq("id", bar.id);
    setSaving(false);
  ...
  return (
    <div className="grid gap-3 md:grid-cols-3">
      <div><Label>Nome</Label><Input value={name} disabled={!isGestor} onChange={(e) => setName(e.target.value)} /></div>
      <div><Label>Apoio responsável</Label><Input value={apoio} disabled={!isGestor} onChange={(e) => setApoio(e.target.value)} /></div>
      <div><Label>Observações</Label><Input value={notes} disabled={!isGestor} onChange={(e) => setNotes(e.target.value)} /></div>
      {isGestor && (
        <div className="md:col-span-3">
          <Button size="sm" onClick={save} disabled={saving}><Save className="w-4 h-4 mr-1" />Salvar dados básicos</Button>
        </div>
      )}
    </div>
  );
...
function MachinesEditor({ barId, machines, isGestor, onChanged }: any) {
  ...
  async function save() {
    setSaving(true);
    const rows = [] as any[];
    for (const b of BRANDS) for (const bi of BICOS) {
      const q = state[`${b}_${bi}`] ?? 0;
      rows.push({ bar_id: barId, brand: b, bicos: bi, quantidade: q });
    }
    const { error } = await supabase.from("bar_machines").upsert(rows, { onConflict: "bar_id,brand,bicos" });
    setSaving(false);
    if (error) return toast.error(error.message);
    toast.success("Máquinas atualizadas"); onChanged();
  }
  ...
  return (
    <div className="rounded border border-border p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="font-display text-xs tracking-widest text-muted-foreground">MÁQUINAS DE CHOPP</div>
        <div className="text-xs text-muted-foreground">Total: <b className="text-foreground">{total}</b></div>
      </div>
        ...
        {BRANDS.map((b) => (
          <div key={b} className="rounded border border-border/70 p-3 space-y-2 bg-card/50">
            ...
            <div className="grid grid-cols-2 gap-2">
              {BICOS.map((bi) => (
                <div key={bi}>
                  <Label className="text-[11px] text-muted-foreground">{bi} bico{bi > 1 ? "s" : ""}</Label>
                  <Input
                    type="number" min={0} inputMode="numeric" placeholder="0"
                    value={(state[`${b}_${bi}`] ?? 0) === 0 ? "" : String(state[`${b}_${bi}`])}
                    disabled={!isGestor}
                    onChange={(e) => setState((s) => ({ ...s, [`${b}_${bi}`]: Math.max(0, +e.target.value) }))}
                  />
                </div>
              ))}
            </div>
      ...
      {isGestor && (
        <Button size="sm" onClick={save} disabled={saving}><Save className="w-4 h-4 mr-1" />Salvar máquinas</Button>
      )}
    </div>
  );
}
...
function InfraEditor({ bar, isGestor, onSaved }: any) {
  ...
  async function save() {
    setSaving(true);
    const { error } = await supabase.from("bars").update({
      cilindros_qtd: cil, manometros_qtd: man, pre_resfriadores_qtd: pre,
    }).eq("id", bar.id);
    setSaving(false);
    if (error) return toast.error(error.message);
    toast.success("Infra atualizada"); onSaved();
  }
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {items.map((it) => (
          <div key={it.label} className="rounded border border-border/70 p-3 bg-card/50">
            ...
            <Input type="number" min={0} inputMode="numeric" placeholder="0" value={it.value === 0 ? "" : String(it.value)} disabled={!isGestor}
              onChange={(e) => it.setter(Math.max(0, +e.target.value))} />
          </div>
        ))}
      </div>
      {isGestor && (
        <Button size="sm" onClick={save} disabled={saving}><Save className="w-4 h-4 mr-1" />Salvar infra</Button>
      )}
    </div>
  );
}
...
function CardReaderEditor({ barId, qty, isGestor, onChanged }: any) {
  ...
  async function save() {
    const { error } = await supabase.from("bar_card_readers").upsert({ bar_id: barId, quantidade: v }, { onConflict: "bar_id" });
    if (error) return toast.error(error.message);
    toast.success("Salvo"); onChanged();
  }
  return (
    <div className="rounded border border-border p-3">
      <div className="font-display text-xs tracking-widest text-muted-foreground mb-2">MÁQUINAS DE CARTÃO</div>
      <div className="flex items-center gap-2">
        <Input type="number" min={0} inputMode="numeric" placeholder="0" value={(v ?? 0) === 0 ? "" : String(v)} disabled={!isGestor} onChange={(e) => setV(+e.target.value)} className="w-24" />
        {isGestor && <Button size="sm" onClick={save}>Salvar</Button>}
      </div>
    </div>
  );
}
...
function ShiftsEditor({ barId, shifts, isGestor, onChanged }: any) {
  ...
  async function save(slot: string, value: number) {
    const { error } = await supabase.from("bar_shifts").upsert({ bar_id: barId, shift: slot as any, meninas_qtd: value }, { onConflict: "bar_id,shift" });
    if (error) return toast.error(error.message);
    toast.success("Turno salvo"); onChanged();
  }
  return (
    <div className="rounded border border-border p-3">
      <div className="font-display text-xs tracking-widest text-muted-foreground mb-2">MENINAS POR TURNO</div>
      <div className="space-y-2">
        {SHIFTS.map((s) => (
          <div key={s.v} className="flex items-center gap-2 text-sm">
            <span className="w-24">{s.l}</span>
            <Input type="number" min={0} inputMode="numeric" placeholder="0" value={(local[s.v] ?? 0) === 0 ? "" : String(local[s.v])} disabled={!isGestor}
              onChange={(e) => setLocal((p) => ({ ...p, [s.v]: +e.target.value }))} className="w-20" />
            {isGestor && <Button size="sm" variant="outline" onClick={() => save(s.v, local[s.v] ?? 0)}>OK</Button>}
          </div>
        ))}
      </div>
    </div>
...
function StockStdEditor({ barId, std, isGestor, onChanged }: any) {
  ...
  async function save(brand: Brand, value: number) {
    const { error } = await supabase.from("bar_stock_standard").upsert({ bar_id: barId, brand, barris_padrao: value }, { onConflict: "bar_id,brand" });
    if (error) return toast.error(error.message);
    toast.success("Padrão salvo"); onChanged();
  }
  return (
    <div className="rounded border border-border p-3">
      <div className="font-display text-xs tracking-widest text-muted-foreground mb-2">PADRÃO DE ESTOQUE (BARRIS)</div>
      <div className="space-y-2">
        {BRANDS.map((b) => (
          <div key={b} className="flex items-center gap-2 text-sm">
            <span className="w-24 uppercase">{b}</span>
            <Input type="number" min={0} inputMode="numeric" placeholder="0" value={(local[b] ?? 0) === 0 ? "" : String(local[b])} disabled={!isGestor}
              onChange={(e) => setLocal((p) => ({ ...p, [b]: +e.target.value }))} className="w-20" />
            {isGestor && <Button size="sm" variant="outline" onClick={() => save(b, local[b] ?? 0)}>OK</Button>}
          </div>
        ))}
      </div>
    </div>
...
async function uploadPhoto(barId: string, kind: string, file: File): Promise<string | null> {
  ...
  if (error) { toast.error("Erro upload foto: " + error.message); return null; }
  return path;
}
...
function InventorySection({ barId, stockStd, inventories, lastCounts, onDone }: any) {
    ...
    try {
      ...
      if (typeof navigator !== "undefined" && !navigator.onLine) {
        ...
        await enqueue("inventory.submit", {
          barId, userId: user!.id, itemsLong, photoBase64, notes: notes || null,
        });
        toast.success("💾 Salvo offline — sincroniza ao voltar rede");
        setOpen(false); setFile(null); setNotes("");
        setCounts({ heineken: { plugado: 0, fechado: 0, vazio: 0 }, amstel: { plugado: 0, fechado: 0, vazio: 0 } });
        return;
      }

      const photo = await uploadPhoto(barId, "inventario", file);
      if (!photo) return;
      const { data: inv, error } = await supabase.from("inventories")
        .insert({ bar_id: barId, photo_url: photo, notes: notes || null, performed_by: user!.id }).select().single();
      ...
      if (abaixo.length > 0) {
        toast.warning("⚠ Inventário registrado · estoque abaixo do padrão", {
          description: abaixo.map((b) => `${b.toUpperCase()}: ${(counts[b].plugado || 0) + (counts[b].fechado || 0)} / padrão ${stockStd[b]}`).join(" · "),
        });
      } else {
      ...
      try {
        if (file && (!navigator.onLine || /network|fetch|failed/i.test(err?.message ?? ""))) {
          ...
          await enqueue("inventory.submit", { barId, userId: user!.id, itemsLong, photoBase64, notes: notes || null });
          toast.success("💾 Salvo offline — sincroniza ao voltar rede");
          setOpen(false); setFile(null); setNotes("");
          setCounts({ heineken: { plugado: 0, fechado: 0, vazio: 0 }, amstel: { plugado: 0, fechado: 0, vazio: 0 } });
          return;
      ...
      toast.error("Falha ao salvar inventário", { description: err?.message ?? "Verifique a conexão e tente novamente" });
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Card className="p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-sm tracking-widest text-muted-foreground">INVENTÁRIO (3× AO DIA)</h2>
        <Button size="sm" onClick={() => setOpen((o) => !o)}><Plus className="w-4 h-4 mr-1" />Novo inventário</Button>
      </div>
            ...
            return (
              <div key={b} className="rounded border border-border p-3">
                ...
                <div className="text-[11px] mt-1">
                  Plugado {c?.plugado ?? 0} · Fechado {c?.fechado ?? 0} · Vazio <b className="text-accent">{c?.vazio ?? 0}</b>
                </div>
                {diff !== 0 && (
                  <div className={`text-[11px] mt-1 ${diff < 0 ? "text-destructive" : "text-primary"}`}>
      ...
      {open && (
        <div className="border border-border rounded p-3 space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {STATUSES.map((s) => (
                  <div key={s.v}>
                    ...
                    <Input type="number" min={0} inputMode="numeric" placeholder="0" value={counts[b][s.v] === 0 ? "" : String(counts[b][s.v])}
                      onChange={(e) => setCounts((p) => ({ ...p, [b]: { ...p[b], [s.v]: +e.target.value } }))} />
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-2">
            <Button onClick={submit} disabled={saving}>{saving ? <Loader2 className="animate-spin w-4 h-4" /> : <><Camera className="w-4 h-4 mr-1" />Registrar</>}</Button>
      ...
      <div className="space-y-1">
        ...
        {inventories.length === 0 && <div className="text-xs text-muted-foreground">Sem inventários ainda.</div>}
        {inventories.map((i: any) => (
          <div key={i.id} className="text-xs flex items-center justify-between border-b border-border/40 py-1">
...
function RefillSection({ barId, refills, onDone, stockStd, lastCounts }: any) {
  ...
  async function submit() {
    ...
    if (excedidos.length > 0) {
      const msg = excedidos.map((e) => `${e.brand.toUpperCase()}: ${e.novoTotal} (padrão atual ${e.padrao})`).join("\n");
      atualizarPadrao = window.confirm(`Você deseja alterar o padrão desse bar?\n\nA reposição vai deixar acima do padrão:\n\n${msg}\n\nOK = atualizar padrão · Cancelar = manter padrão`);
    }

    setSaving(true);
    try {
      const itemsPayload = BRANDS.filter((b) => qty[b] > 0).map((b) => ({ brand: b, quantidade: qty[b] }));
      const emptiesPayload = BRANDS.filter((b) => empt[b] > 0).map((b) => ({ brand: b, quantidade: empt[b] }));

      if (typeof navigator !== "undefined" && !navigator.onLine) {
        const photoBase64 = await fileToBase64(file);
        await enqueue("refill.submit", {
          barId, userId: user!.id, items: itemsPayload, empties: emptiesPayload, photoBase64, notes: notes || null,
        });
        toast.success("💾 Reposição salva offline — sincroniza ao voltar rede");
        setOpen(false); setFile(null); setNotes("");
        setQty({ heineken: 0, amstel: 0 }); setEmpt({ heineken: 0, amstel: 0 });
        return;
      }

      const photo = await uploadPhoto(barId, "reposicao", file);
      if (!photo) return;
      const { data: rf, error } = await supabase.from("refills")
        .insert({ bar_id: barId, photo_url: photo, notes: notes || null, performed_by: user!.id }).select().single();
      if (error) throw error;
      ...
      const emps = emptiesPayload.map((e) => ({
        bar_id: barId, refill_id: rf.id, brand: e.brand, quantidade: e.quantidade, performed_by: user!.id,
      }));
      if (emps.length) {
      ...
      if (atualizarPadrao && excedidos.length > 0) {
        for (const e of excedidos) {
          await supabase.from("bar_stock_standard")
            .upsert({ bar_id: barId, brand: e.brand, barris_padrao: e.novoTotal }, { onConflict: "bar_id,brand" });
        }
        setFlash(`⚡ Padrão alterado: ${excedidos.map((e) => `${e.brand} → ${e.novoTotal}`).join(" · ")}`);
        toast.success("⚡ Padrão alterado", {
          description: excedidos.map((e) => `${e.brand.toUpperCase()}: ${e.novoTotal} barris`).join(" · "),
        });
      ...
      try {
        if (file && (!navigator.onLine || /network|fetch|failed/i.test(err?.message ?? ""))) {
          ...
          const itemsPayload = BRANDS.filter((b) => qty[b] > 0).map((b) => ({ brand: b, quantidade: qty[b] }));
          const emptiesPayload = BRANDS.filter((b) => empt[b] > 0).map((b) => ({ brand: b, quantidade: empt[b] }));
          await enqueue("refill.submit", { barId, userId: user!.id, items: itemsPayload, empties: emptiesPayload, photoBase64, notes: notes || null });
          toast.success("💾 Reposição salva offline — sincroniza ao voltar rede");
          setOpen(false); setFile(null); setNotes("");
          setQty({ heineken: 0, amstel: 0 }); setEmpt({ heineken: 0, amstel: 0 });
          return;
      ...
      toast.error("Falha ao registrar reposição", { description: err?.message ?? "Verifique a conexão e tente novamente" });
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Card className="p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-sm tracking-widest text-muted-foreground">REPOSIÇÃO / ABASTECIMENTO</h2>
        <Button size="sm" onClick={() => setOpen((o) => !o)}><Plus className="w-4 h-4 mr-1" />Nova reposição</Button>
      </div>
      {flash && (
            ...
            {BRANDS.map((b) => (
              <div key={b} className="space-y-2">
                ...
                <div>
                  ...
                  <Input type="number" min={0} inputMode="numeric" placeholder="0" value={qty[b] === 0 ? "" : String(qty[b])} onChange={(e) => setQty((p) => ({ ...p, [b]: +e.target.value }))} />
                </div>
                <div>
                  <Label className="text-[11px]">Vazios retirados</Label>
                  <Input type="number" min={0} inputMode="numeric" placeholder="0" value={empt[b] === 0 ? "" : String(empt[b])} onChange={(e) => setEmpt((p) => ({ ...p, [b]: +e.target.value }))} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">Histórico</div>
        {refills.length === 0 && <div className="text-xs text-muted-foreground">Sem reposições ainda.</div>}
        {refills.map((r: any) => (
          <div key={r.id} className="text-xs border-b border-border/40 py-1">
            <div className="flex justify-between">
              ...
              <span className="text-muted-foreground">
                {(r.refill_items ?? []).map((it: any) => `${it.quantidade} ${it.brand}`).join(" · ") || "sem itens"}
              </span>
            </div>
          </div>
        ))}
...
function CollectEmptiesSection({ barId, onDone }: { barId: string; onDone: () => void }) {
  ...
  async function submit() {
    ...
    try {
      const rowsPayload = BRANDS.filter((b) => empt[b] > 0).map((b) => ({
        brand: b, quantidade: empt[b], refill_id: null,
      }));

      if (typeof navigator !== "undefined" && !navigator.onLine) {
        const photoBase64 = await fileToBase64(file);
        await enqueue("empties.remove", {
          barId, userId: user!.id, rows: rowsPayload, photoBase64, notes: notes || null,
        });
        toast.success("💾 Salvo offline — sincroniza ao voltar rede");
        setOpen(false); setFile(null); setNotes("");
      ...
      const rows = rowsPayload.map((r) => ({
        ...r, bar_id: barId, photo_url: photo, notes: notes || null, performed_by: user!.id,
      }));
      ...
      try {
        if (file && (!navigator.onLine || /network|fetch|failed/i.test(e?.message ?? ""))) {
          const photoBase64 = await fileToBase64(file);
          const rowsPayload = BRANDS.filter((b) => empt[b] > 0).map((b) => ({ brand: b, quantidade: empt[b], refill_id: null }));
          await enqueue("empties.remove", { barId, userId: user!.id, rows: rowsPayload, photoBase64, notes: notes || null });
          toast.success("💾 Salvo offline — sincroniza ao voltar rede");
          setOpen(false); setFile(null); setNotes("");
          setEmpt({ heineken: 0, amstel: 0 });
          return;
  ...
  return (
    <Card className="p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-sm tracking-widest text-muted-foreground">RECOLHER VAZIOS</h2>
        <Button size="sm" variant="secondary" onClick={() => setOpen((o) => !o)}>
          <Plus className="w-4 h-4 mr-1" />Recolher vazios
        </Button>
      </div>
        ...
        <div className="border border-border rounded p-3 space-y-3">
          ...
          <div className="flex gap-2">
            <Button onClick={submit} disabled={saving}>
              {saving ? <Loader2 className="animate-spin w-4 h-4" /> : <><Camera className="w-4 h-4 mr-1" />Registrar</>}
            </Button>
            <Button variant="ghost" onClick={() => setOpen(false)}>Cancelar</Button>
          </div>
        </div>
      ...
      <div className="space-y-1">
        ...
        {history.length === 0 && <div className="text-xs text-muted-foreground">Nenhum recolhimento avulso.</div>}
        {history.map((h) => (
          <div key={h.id} className="text-xs border-b border-border/40 py-1 flex justify-between">
            ...
            <span className="text-muted-foreground">{h.quantidade} {h.brand}</span>
          </div>
        ))}
      </div>
    </Card>
...
function MissionsSection({ barId }: { barId: string }) {
  ...
  async function load() {
    ...
    const [{ data: t }, { data: o }] = await Promise.all([
      supabase.from("bar_temperature_checks").select("*").eq("bar_id", barId).order("performed_at", { ascending: false }).limit(20),
      supabase.from("bar_organization_checks").select("*").eq("bar_id", barId).order("performed_at", { ascending: false }).limit(20),
    ]);
    setTemps(t ?? []);
    setOrgs(o ?? []);
    setLoading(false);
  ...
  temps.forEach((t) => {
    if (new Date(t.performed_at).toDateString() === todayKey && !lastBySlot[t.slot]) lastBySlot[t.slot] = t;
  });
  const lastOrg = orgs[0];
  const lastOrgToday = lastOrg && new Date(lastOrg.performed_at).toDateString() === todayKey ? lastOrg : null;

  return (
    <div className="space-y-4">
      <TemperatureMission barId={barId} temps={temps} lastBySlot={lastBySlot} onDone={load} loading={loading} />
...
function TemperatureMission({ barId, temps, lastBySlot: lastBySlotProp, onDone, loading }: any) {
  ...
  async function submit() {
    ...
    setSaving(true);
    try {
      const optimisticRow = { temperatura: Number(temp), performed_at: new Date().toISOString(), _pending: true };
      if (typeof navigator !== "undefined" && !navigator.onLine) {
        const photoBase64 = await fileToBase64(file);
        await enqueue("temperature.submit", {
          barId, userId: user.id, slot, temperatura: Number(temp), photoBase64,
        });
        setOptimistic((o) => ({ ...o, [slot]: optimisticRow }));
        toast.success("💾 Salvo offline — sincroniza ao voltar rede");
        setSlot(""); setTemp(""); setFile(null);
        return;
      }
      const path = await uploadPhoto(barId, "temperatura", file);
      if (!path) return;
      const { error } = await supabase.from("bar_temperature_checks").insert({
        bar_id: barId, slot: slot as any, temperatura: Number(temp),
        photo_url: path, performed_by: user.id,
      });
      if (error) return toast.error(error.message);
      setOptimistic((o) => ({ ...o, [slot]: optimisticRow }));
      ...
      try {
        if (file && (!navigator.onLine || /network|fetch|failed/i.test(err?.message ?? ""))) {
          const photoBase64 = await fileToBase64(file);
          await enqueue("temperature.submit", { barId, userId: user.id, slot, temperatura: Number(temp), photoBase64 });
          setOptimistic((o) => ({ ...o, [slot]: { temperatura: Number(temp), performed_at: new Date().toISOString(), _pending: true } }));
          toast.success("💾 Salvo offline — sincroniza ao voltar rede");
          setSlot(""); setTemp(""); setFile(null);
          return;
        }
  ...
  return (
    <Card className="p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-display text-sm tracking-widest text-muted-foreground">TEMPERATURA DO CHOPP</h2>
      </div>
      <p className="text-[11px] text-muted-foreground">
        Meça às 11h, 17h e 22h. Ideal: <b className="text-primary">≤ {IDEAL_TEMP}°C</b> — foto do termômetro obrigatória.
      </p>
      ...
      <div className="grid grid-cols-3 gap-2">
        {TEMP_SLOTS.map((s) => {
          ...
          return (
            <div key={s.v} className={`rounded border p-2 text-center ${v.seal ? "border-primary bg-primary/10" : "border-accent bg-accent/10"}`}>
              <div className="font-display text-sm">{s.l}</div>
              <div className="font-display text-lg">{Number(rec.temperatura).toFixed(1)}°C</div>
              <div className={`text-[9px] tracking-widest ${v.seal ? "text-primary" : "text-accent"}`}>
      ...
      <div className="rounded border border-border p-3 space-y-2">
        <div className="text-[11px] font-display tracking-widest text-muted-foreground">NOVO REGISTRO</div>
        <div className="grid grid-cols-3 gap-2">
          {TEMP_SLOTS.map((s) => (
            <Button key={s.v} type="button" size="sm"
              variant={slot === s.v ? "default" : "outline"}
              onClick={() => setSlot(s.v)}>{s.l}</Button>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div>
        ...
        {temp && (
          <div className="text-[11px]">
            Prévia: <b className={tempVerdict(Number(temp)).seal ? "text-primary" : "text-accent"}>
              {tempVerdict(Number(temp)).label}
            </b>
          </div>
        )}
        <Button size="sm" onClick={submit} disabled={saving} className="w-full">
          {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-1" />Registrar</>}
        </Button>
      </div>
      ...
      <div className="space-y-1">
        ...
        {!loading && temps.length === 0 && <div className="text-xs text-muted-foreground">Sem medições ainda.</div>}
        {temps.slice(0, 8).map((r: any) => {
          ...
          return (
            <div key={r.id} className="text-xs border-b border-border/40 py-1 flex justify-between">
              <span>{new Date(r.performed_at).toLocaleString("pt-BR")} · {slotL}</span>
...
function OrganizationMission({ barId, orgs, lastOrgToday: lastOrgTodayProp, onDone }: any) {
  ...
  const [state, setState] = useState<Record<string, boolean>>({
    copo_ok: false, meninas_ok: false, limpo_ok: false, sem_fila_ok: false,
  });
  const [saving, setSaving] = useState(false);
  ...
  async function submit() {
    ...
    try {
      ...
      const { error } = await supabase.from("bar_organization_checks").insert({
        bar_id: barId, ...state, performed_by: user.id,
      });
      if (error) return toast.error(error.message);
  ...
  return (
    <Card className="p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-display text-sm tracking-widest text-muted-foreground">ORGANIZAÇÃO DO BAR</h2>
      </div>

      {lastOrgToday && (
        <div className={`rounded p-3 text-center border ${lastAllOk ? "border-primary bg-primary/10" : "border-border bg-card/40"}`}>
          {lastAllOk ? (
            <>
              <Award className="w-6 h-6 mx-auto text-primary" />
              <div className="font-display text-sm text-primary tracking-widest mt-1">★ PADRÃO DISPEL · BAR TOP</div>
              <div className="text-[10px] text-muted-foreground">
                {new Date(lastOrgToday.performed_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
              </div>
            </>
          ) : (
            <div className="text-xs text-muted-foreground">
              Último check às {new Date(lastOrgToday.performed_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} — pendências restantes
            </div>
          )}
        </div>
      )}
      ...
      <div className="space-y-2">
        {ORG_ITEMS.map((i) => (
          <label key={i.k}
            ...
            <input type="checkbox" checked={state[i.k]}
      ...
      {allOk && (
        <div className="rounded border border-primary bg-primary/10 p-2 text-center">
          ...
          <span className="font-display text-xs tracking-widest text-primary">★ PADRÃO DISPEL · BAR TOP</span>
        </div>
      )}

      <Button size="sm" onClick={submit} disabled={saving} className="w-full">
        {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-1" />Registrar check</>}
      </Button>

      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">Histórico</div>
        {orgs.length === 0 && <div className="text-xs text-muted-foreground">Sem checks ainda.</div>}
...
function StaffSection({ barId, shifts, cardQty, userId }: any) {
  ...
  useEffect(() => { load(); const t = setInterval(() => setNow(new Date()), 60_000); return () => clearInterval(t); /* eslint-disable-next-line */ }, [barId]);
    ...
    <div className="space-y-3">
      <Card className="p-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display text-sm tracking-widest text-muted-foreground">AGORA · {now.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</h2>
          {active.length > 1 && <Badge className="bg-accent text-accent-foreground">TURNOS ACUMULADOS</Badge>}
        </div>
        {active.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded border border-border p-3">
            <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground"><Users className="w-3 h-3" /> Meninas esperadas</div>
          ...
          <div className="rounded border border-border p-3">
            <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground"><CreditCard className="w-3 h-3" /> Maquininhas</div>
      ...
      <Card className="p-4 space-y-3">
        <h2 className="font-display text-sm tracking-widest text-muted-foreground">CHECKS DE HOJE (3×)</h2>
        {loading && <div className="text-xs text-muted-foreground">Carregando…</div>}
        {CHECKPOINTS.map((cp) => {
          ...
          return (
            <div key={cp.n} className="border border-border rounded p-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-display text-sm tracking-wider">{cp.label} <span className="text-[11px] text-muted-foreground">· {cp.hint}</span></div>
                  {done ? (
                    <div className="text-[11px] text-muted-foreground mt-1">
                      {new Date(done.performed_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} · {done.meninas_count} meninas · {done.card_readers_count} maq.
                    </div>
                  ) : (
                ...
                <div className="flex items-center gap-2">
                  {done ? (
                    ok ? <Badge className="bg-primary text-primary-foreground"><CheckCircle2 className="w-3 h-3 mr-1" />OK</Badge>
                       : <Badge variant="destructive"><AlertTriangle className="w-3 h-3 mr-1" />AJUSTAR</Badge>
                  ) : (
                    <Button size="sm" onClick={() => setOpenCP(cp.n)}>Registrar</Button>
                  )}
                </div>
              </div>
              {openCP === cp.n && (
...
function CheckForm({ barId, userId, checkpoint, expectedMeninas, expectedCards, onCancel, onSaved }: any) {
  ...
  return (
    <div className="mt-3 space-y-2 border-t border-border pt-3">
      <div className="grid grid-cols-2 gap-2">
        <div>
          <Label className="text-xs">Meninas contadas <span className="text-muted-foreground">(esp. {expectedMeninas})</span></Label>
          <Input type="number" inputMode="numeric" value={meninas === 0 ? "" : String(meninas)} onChange={(e) => setMeninas(Number(e.target.value) || 0)} />
        </div>
        <div>
          <Label className="text-xs">Maquininhas <span className="text-muted-foreground">(esp. {expectedCards})</span></Label>
          <Input type="number" inputMode="numeric" value={cards === 0 ? "" : String(cards)} onChange={(e) => setCards(Number(e.target.value) || 0)} />
        </div>
      </div>
      <div>
      ...
      <div className="flex gap-2">
        ...
        <Button size="sm" onClick={save} disabled={saving} className="flex-1">
          {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-1" />Salvar check</>}
        </Button>
      </div>
    </div>
  );
...
function TransferSection({ barId, onDone }: { barId: string; onDone: () => void }) {
  ...
  async function load() {
    const [{ data: bs }, { data: outT }, { data: inT }] = await Promise.all([
      ...
      supabase.from("bar_transfers").select("*, bar_transfer_items(*), to_bar:bars!bar_transfers_to_bar_id_fkey(name)").eq("from_bar_id", barId).order("performed_at", { ascending: false }).limit(20),
      supabase.from("bar_transfers").select("*, bar_transfer_items(*), from_bar:bars!bar_transfers_from_bar_id_fkey(name)").eq("to_bar_id", barId).order("performed_at", { ascending: false }).limit(20),
    ]);
    setBars(bs ?? []);
    const merged = [
  ...
  async function submit() {
    ...
    try {
      ...
      const { data: tr, error } = await supabase.from("bar_transfers").insert({
        from_bar_id, to_bar_id, photo_url: photo, notes: notes || null, performed_by: user!.id,
      }).select().single();
      if (error) return toast.error(error.message);
      const items = BRANDS.filter((b) => qty[b] > 0).map((b) => ({ transfer_id: tr.id, brand: b, quantidade: qty[b] }));
      if (items.length) {
      ...
      const fromName = direction === "out" ? "este bar" : (bars.find((x) => x.id === otherBarId)?.name ?? "outro bar");
      const toName = direction === "out" ? (bars.find((x) => x.id === otherBarId)?.name ?? "outro bar") : "este bar";
      const refillNotes = `__transfer__ Recebido via transferência de ${fromName}`;
      const { data: rf, error: rfErr } = await supabase.from("refills")
        .insert({ bar_id: to_bar_id, photo_url: photo, notes: refillNotes, performed_by: user!.id })
        .select().single();
      if (!rfErr && rf) {
        const refillItems = BRANDS.filter((b) => qty[b] > 0).map((b) => ({ refill_id: rf.id, brand: b, quantidade: qty[b] }));
      ...
      await supabase.from("refills").insert({
        bar_id: from_bar_id,
        photo_url: photo,
        notes: `__transfer__ Saída via transferência para ${toName}: ${BRANDS.filter((b) => qty[b] > 0).map((b) => `${qty[b]} ${b}`).join(" · ")}`,
        performed_by: user!.id,
      });

      toast.success(direction === "out" ? "✓ Baixa registrada · destino reposto" : "✓ Entrada registrada como reposição");

      setOpen(false); setFile(null); setNotes(""); setOtherBarId("");
      setQty({ heineken: 0, amstel: 0 });
      load();
      onDone();
    } catch (err: any) {
      toast.error("Falha ao registrar transferência", { description: err?.message ?? "Verifique a conexão" });
    } finally {
      setSaving(false);
    }
  }
    ...
    <Card className="p-4 space-y-3">
      ...
      {open && (
        <div className="border border-border rounded p-3 space-y-3">
          ...
          <div>
            <Label className="text-[11px]">{direction === "out" ? "Bar destino" : "Bar origem"} *</Label>
            <select
          ...
          <div>
            <Label>Observações</Label>
            <Input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="ex: transferi 3 heineken para o Villa 2" />
          </div>
          <div className="flex gap-2">
            <Button onClick={submit} disabled={saving}>
              {saving ? <Loader2 className="animate-spin w-4 h-4" /> : <><ArrowRightLeft className="w-4 h-4 mr-1" />Registrar transferência</>}
            </Button>
            <Button variant="ghost" onClick={() => setOpen(false)}>Cancelar</Button>
          </div>
        </div>
      ...
      <div className="space-y-1">
        <div className="text-xs text-muted-foreground">Histórico</div>
        {transfers.length === 0 && <div className="text-xs text-muted-foreground">Sem transferências ainda.</div>}
        {transfers.map((t: any) => {
          const isOut = t._dir === "out";
          const other = isOut ? t.to_bar?.name : t.from_bar?.name;
          const items = (t.bar_transfer_items ?? []).map((it: any) => `${it.quantidade} ${it.brand}`).join(" · ");
          return (
            <div key={t.id} className="text-xs border-b border-border/40 py-1 flex justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="text-muted-foreground">{new Date(t.performed_at).toLocaleString("pt-BR")}</div>
              </div>
