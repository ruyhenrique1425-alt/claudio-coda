...
import {
  ArrowLeft, CreditCard, Camera, Loader2, Save, Package, PackageCheck,
...
function EquipeBarDetail() {
  ...
  useEffect(() => {
    supabase.from("bars").select("*").eq("id", barId).maybeSingle().then(({ data }) => {
      setBar(data);
      setLoading(false);
    });
  }, [barId]);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-4 pb-24">
      <Link to="/app/equipe-bar" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground mb-3">
      ...
      <div className="text-xs text-muted-foreground mb-4">Equipe de bar · {new Date().toLocaleDateString("pt-BR")}</div>
...
function PromotorasTab({ barId, userId }: any) {
  ...
  async function load() {
    ...
    const [{ data: sh }, { data: rd }, { data: ck }] = await Promise.all([
      ...
      supabase.from("bar_staff_checks").select("*").eq("bar_id", barId).gte("performed_at", start.toISOString()).order("performed_at"),
    ]);
    const map: Record<string, number> = {};
    (sh ?? []).forEach((r: any) => { map[r.shift] = r.meninas_qtd ?? 0; });
    setShifts(map);
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded border border-border p-3">
            <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground"><Users className="w-3 h-3" /> Promotoras esperadas</div>
          ...
          <div className="rounded border border-border p-3">
            <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground"><CreditCard className="w-3 h-3" /> Maquininhas padrão</div>
                ...
                <div>
                  ...
                  {done ? (
                    <div className="text-[11px] text-muted-foreground mt-1">
                      {new Date(done.performed_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} · {done.meninas_count} promotoras · {done.card_readers_count} maq.
                    </div>
                  ) : (
...
function StaffCheckForm({ barId, userId, checkpoint, expectedMeninas, expectedCards, onCancel, onSaved }: any) {
  ...
  return (
    <div className="mt-3 space-y-2 border-t border-border pt-3">
      <div className="grid grid-cols-2 gap-2">
        <div>
          <Label className="text-xs">Promotoras contadas <span className="text-muted-foreground">(esp. {expectedMeninas})</span></Label>
...
function MaquininhasTab({ barId, userId }: any) {
  ...
  async function load() {
    ...
    const [{ data: r }, { data: s }, { data: reg }] = await Promise.all([
      ...
      supabase.from("bar_card_machine_sessions").select("*").eq("bar_id", barId).eq("event_date", todayISO()).order("picked_up_at"),
      supabase.from("bar_machine_patrimonios").select("patrimonio,ativo").eq("bar_id", barId).eq("ativo", true).order("patrimonio"),
    ]);
  ...
  if (loading) return <div className="p-6 text-center text-muted-foreground text-sm">Carregando…</div>;
    ...
    <div>
      <div className="grid grid-cols-4 gap-2 mb-4">
        ...
        <Card className="p-2 text-center">
          ...
          <div className={`font-display text-xl ${faltaRetirar > 0 ? "text-destructive" : "text-primary"}`}>{faltaRetirar}</div>
        </Card>
        <Card className="p-2 text-center">
      ...
      <PickupForm barId={barId} userId={userId} suggestion={faltaRetirar} onSaved={load} registry={registry} jaHoje={jaHoje} />
      ...
      <div className="mt-5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h2 className="font-display text-sm tracking-widest text-muted-foreground">EM CAMPO ({abertas.length})</h2>
        </div>
        {abertas.length === 0 && <Card className="p-3 text-xs text-muted-foreground text-center">Nenhuma maquininha em campo.</Card>}
      ...
      {fechadas.length > 0 && (
        <div className="mt-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-display text-sm tracking-widest text-muted-foreground">DEVOLVIDAS ({fechadas.length})</h2>
          </div>
          <div className="space-y-2">
            {fechadas.map((s) => (
              <SessionCard key={s.id} s={s} urls={urls} userId={userId} onSaved={load} />
      ...
      <Card className={`mt-6 p-4 ${faltaDevolver === 0 && sessions.length > 0 ? "border-primary" : "border-destructive"}`}>
        <div className="flex items-center gap-2">
          {faltaDevolver === 0 && sessions.length > 0 ? (
            <><CheckCircle2 className="w-5 h-5 text-primary" /><span className="font-display text-sm tracking-wider text-primary">TODAS AS MAQUININHAS DEVOLVIDAS</span></>
          ) : (
            <><AlertTriangle className="w-5 h-5 text-destructive" /><span className="font-display text-sm tracking-wider text-destructive">FALTA DEVOLVER {faltaDevolver} MAQUININHA{faltaDevolver !== 1 ? "S" : ""}</span></>
          )}
        </div>
      </Card>
    </div>
...
function PickupForm({ barId, userId, suggestion, onSaved, registry = [], jaHoje = new Set() }: any) {
  ...
  function toggleChip(p: string) {
    const key = p.trim();
    const parts = patrimonio.split(";").map((x) => x.trim()).filter(Boolean);
    const idx = parts.findIndex((x) => x.toUpperCase() === key.toUpperCase());
    if (idx >= 0) parts.splice(idx, 1); else parts.push(key);
  ...
  const selecionados = new Set(
    patrimonio.split(";").map((x) => x.trim().toUpperCase()).filter(Boolean)
  );

  async function save() {
    if (!userId) return toast.error("Faça login");
    const lista = patrimonio.split(";").map((p) => p.trim()).filter(Boolean);
    ...
    setSaving(false);
    if (error) return toast.error(error.message);
    toast.success(lista.length === 1 ? "Máquina registrada" : `${lista.length} máquinas registradas`);
    setPatrimonio(""); setFile(null); setNotes("");
    onSaved();
  }
  ...
  return (
    <Card className="p-4 space-y-3 border-accent/40">
      <div className="flex items-center justify-between">
        <div className="font-display text-sm tracking-widest text-accent">RETIRAR MAQUININHA (MEEP)</div>
        {suggestion > 0 && <Badge variant="outline" className="text-[10px]">sugerido +{suggestion}</Badge>}
      </div>
      ...
      {registry.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={selectAllPending} className="text-[11px] text-primary underline">
      ...
      <div>
        <Label className="text-xs">Patrimônio <span className="text-muted-foreground">(separe várias com ; )</span></Label>
        <Input value={patrimonio} onChange={(e) => setPatrimonio(e.target.value)} placeholder="Ex: PAG 48033;PAG 43953" autoCapitalize="characters" />
      </div>
      <div>
      ...
      <div>
        <Label className="text-xs">Observações</Label>
        <Textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Troca, reposição, etc. (opcional)" />
      </div>
      <Button onClick={save} disabled={saving} className="w-full">
        {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Camera className="w-4 h-4 mr-2" />Registrar retirada</>}
      </Button>
    </Card>
  );
}
...
function SessionCard({ s, urls, userId, onSaved }: any) {
  ...
  async function doReturn() {
    ...
    if (!photo) { setSaving(false); return; }
    const { error } = await supabase.from("bar_card_machine_sessions").update({
    ...
    setOpenReturn(false); setFile(null);
    onSaved();
  }
  ...
  return (
    <Card className="p-3">
      <div className="flex items-center gap-3">
        {upUrl && <a href={upUrl} target="_blank" rel="noreferrer"><img src={upUrl} className="w-12 h-12 rounded object-cover" /></a>}
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {s.returned_at
              ...
              : <Badge variant="outline" className="text-[10px] border-accent text-accent">EM CAMPO</Badge>}
          </div>
          <div className="text-[11px] text-muted-foreground mt-0.5">
            Retirada {new Date(s.picked_up_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
            {s.returned_at && <> · Devolvida {new Date(s.returned_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</>}
          </div>
          {s.notes && <div className="text-[11px] italic text-muted-foreground mt-0.5">{s.notes}</div>}
        </div>
        {retUrl && <a href={retUrl} target="_blank" rel="noreferrer"><img src={retUrl} className="w-12 h-12 rounded object-cover" /></a>}
        {!s.returned_at && !openReturn && (
          <Button size="sm" variant="outline" onClick={() => setOpenReturn(true)}>Devolver</Button>
        )}
      </div>
      {openReturn && (
        <div className="mt-3 border-t border-border pt-3 space-y-2">
          ...
          <Input type="file" accept="image/*" capture="environment" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
          <div className="flex gap-2">
            <Button size="sm" variant="ghost" onClick={() => { setOpenReturn(false); setFile(null); }} disabled={saving}>Cancelar</Button>
            <Button size="sm" onClick={doReturn} disabled={saving} className="flex-1">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4 mr-1" />Confirmar devolução</>}
            </Button>
          </div>
        </div>
      )}
